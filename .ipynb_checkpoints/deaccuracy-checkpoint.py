...
class DeAccuracy():
    ...
    def __init__(self, building: Building):
        ...
        self.output_dir = PREPARED_DIR / building.name
        os.makedirs(self.output_dir, exist_ok=True)
    
    def reduce_csv(self, csv_filename):
        output = PREPARED_DIR / self.building.name / (csv_filename + ".pkl.gz")
        if os.path.exists(output):
            ...
            try:
                df.time = df.time.apply(
                    lambda x: pd.Timestamp(x, tz='America/New_York')
                        .round(freq="T", ambiguous=True, nonexistent="shift_forward")
                )
            ...
            print(emoji.emojize(f":right_arrow_curving_up: {csv_filename} Exporting..."))
            df.to_pickle(output)
            print(emoji.emojize(f":check_mark_button: {csv_filename} Saved!"))
        else:
            print(emoji.emojize(f":prohibited: Cannot read {csv_filename}! It doesn't exist!"))
